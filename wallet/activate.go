...
// contextBlockCounter is the optional context-aware reachability surface: a
...
// RPC timeout. Contract: the implementation must return as soon as ctx is done —
// probeReachable waits for the probe goroutine on timeout, and that wait is
// bounded so even a non-conforming implementation cannot hang the probe.
type contextBlockCounter interface {
...
// xbridgewalletconnectorbtc.cpp:1513-1525). When the connector supports
// GetBlockCountContext, the probe runs under a cancellable context: on timeout
// the in-flight RPC is aborted and the probe goroutine is waited for (bounded)
// before probeReachable returns, so a hung wallet cannot leave a goroutine alive
// up to the wallet RPC timeout. Connectors without the ctx-aware method
// (LocalConnector, test stubs) fall back to the bounded select.
func probeReachable(c Connector) error {
	type result struct{ err error }
	done := make(chan result, 1)
	if bc, ok := c.(contextBlockCounter); ok {
		...
		case r := <-done:
			return r.err
		case <-ctx.Done():
			// The context aborts the in-flight RPC; give the goroutine a bounded
			// window to exit so nothing outlives the probe (a non-conforming
			// implementer that ignores ctx cannot hang the caller).
			joinTimer := time.NewTimer(probeTimeout)
			select {
			case <-done:
				joinTimer.Stop()
			case <-joinTimer.C:

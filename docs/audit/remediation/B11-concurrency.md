...
- **Handshake stall bound.** The dial phase is ctx-cancellable; a peer that
  accepts TCP but stalls the version exchange holds a discovery `Close` up to
  `handshakeTimeout` (60 s), bounded and non-additive (peers run concurrently).
  `Start` must not race `Close`.
- **Coin-registry residual (CONC-F94).** Workers still resolve `coins.Get(cur)`
  at build time via the atomic registry; a reload that drops a coin mid-task
  fails the build with a clean "unknown coin" error rather than redirecting it.
